package com.cheng.shop.user.web.servlet;

import cn.itcast.commons.CommonUtils;
import cn.itcast.servlet.BaseServlet;
import com.cheng.shop.user.domain.User;
import com.cheng.shop.user.service.UserService;
...
public class UserServlet extends BaseServlet {
     ...
     * @throws IOException
     */
    public String validateVerifyCode(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        /*
         * 1.获取输入框种的验证码
         */
    ...
    public String regist(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        /**
         * 2.对表单数据进行服务端校验，校验失败，保存信息，返回到regist.jsp显示
         */
    ...
    public String login(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
         ...
         *   3).转发到login.jsp
         * 6.登陆成功
         *   1).保存当前查询出的User到session中
         *   2).保存当前用户的名称到cookie中，注意中文需要编码处理
         */
    ...
    public String changePwd(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException, UserException {
        /**
         * 1.封装表单数据到user中
         * 2.从session中获取id
         * 3.使用uid和表单中的loginpass和newpass来调用service
         *     如果出现异常，保存异常到request中，转发到pwd.jsp
         * 4.保存成功信息到request中
         * 5.转发到msg.jsp
         */
        User formUser=CommonUtils.toBean(req.getParameterMap(),User.class);
        Map<String,String> errors=validateChangePwd(formUser,req.getSession());
        ...
        if(errors.size()>0){
            req.setAttribute("error",errors);
            req.setAttribute("user",formUser);
            return "f:/jsps/user/changePwd.jsp";
        }
    ...
    private Map<String,String> validateChangePwd(User formUser, HttpSession session) {
        Map<String, String> errors = new HashMap<String, String>();
        String verifyCode=formUser.getVerifyCode();
        String vcode= (String) session.getAttribute("vCode");
        ...
        if (newloginpass==null||newloginpass.trim().isEmpty()) {//非空校验
            errors.put("newloginpass","新密码不能为空");
        }else if(newloginpass.length()<3||newloginpass.length()>20){
            errors.put("newloginpass","新密码长度必须在3到20位之间");
        }
